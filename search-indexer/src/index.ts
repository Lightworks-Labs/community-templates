...
interface SearchEntry {
  ...
  path: string;
  title: string;
  type: string; // collection name (e.g. 'requirements', 'tests') or 'page'
 ...
 */
function buildCollectionMap(contentDir: string): Map<string, string> {
...
function processFile(
  ...
  if (path.basename(filePath) === '_schema.json') return null;
...
function loadConfig(configPath: string, contentDir: string): IndexConfig {
  if (fs.existsSync(configPath)) {
    try {
      const parsed = JSON.parse(fs.readFileSync(configPath, 'utf-8'));
      console.log(`  Using config from ${configPath}`);
      const normalized: IndexConfig = { ...DEFAULT_CONFIG, ...parsed };
      ...
      if (contentBasename && contentBasename !== '.') {
        ...
        normalized.include = normalized.include.map(p =>
          p.startsWith(prefix) ? p.slice(prefix.length) : p
        );
        normalized.exclude = normalized.exclude.map(p =>
          p.startsWith(prefix) ? p.slice(prefix.length) : p
        );
...
async function buildIndex(contentDir: string, outputDir: string): Promise<void> {
  ...
  const master: MetadataIndex = { version: config.version, generatedAt, entries };
  fs.writeFileSync(
  ...
  for (const [collection, colEntries] of byCollection) {
    const colIndex: MetadataIndex = { version: config.version, generatedAt, entries: colEntries };
    fs.writeFileSync(
